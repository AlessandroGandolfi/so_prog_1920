...
void init_players(char *mode) {
    ...
    init_params_players(mode, param_giocatori, tmp_params);
    
    create_players(param_giocatori, tmp_params);
}

void init_params_players(char* mode, char ** param_giocatori, char tmp_params[][sizeof(char *)]){
    ...
    /* TODO CONTROLLARE */
    snprintf(tmp_params[5], 12, "%d", sm_id_cb);
    param_giocatori[7] = tmp_params[5];
    param_giocatori[8] = NULL;

}

void create_players(char ** param_giocatori, char tmp_params[][sizeof(char *)]){
    int i;
...
void wait_flag_taken(int num_band){
    msg_c_flag msg_presa;
    int i;
    for(i = 0; i < num_band; i++) {
        msgrcv(msg_id_queue, &msg_presa, sizeof(msg_c_flag) - sizeof(long), (long) (getpid() + MSG_FLAG), 0);
        TEST_ERROR;

        if(!sm_flags[msg_presa.id_flag].captured) {
            #if DEBUG
            printf("band %d presa da gioc %d, %d punti\n"
                , msg_presa.id_flag
                , (msg_presa.pos_token + 1)
                , sm_flags[msg_presa.id_flag].points);
            #endif

            sm_flags[msg_presa.id_flag].captured = TRUE;
            players[msg_presa.pos_token].points += sm_flags[msg_presa.id_flag].points;
        } else i--;
    }
}
...
int init_flags() {
    ...
    sm_id_flags = shmget(IPC_PRIVATE, num_band * sizeof(flag), S_IRUSR | S_IWUSR);
    TEST_ERROR;
    ...
    for(i = 0; i < num_band; i++) {
        ...
        set_flags_pos_points(i, num_band, tot_punti_rim, box);
    }
...
void set_flags_pos_points(int i, int num_band, int tot_punti_rim, coord box){
    /* valorizzazione mc bandiere */
    sm_flags[i].position = box;
    sm_flags[i].captured = FALSE;
    ...
    */
    if(i != (num_band - 1))
        sm_flags[i].points = rand() % (tot_punti_rim / 2);
    else
        sm_flags[i].points = tot_punti_rim;
    tot_punti_rim -= sm_flags[i].points;
...
void send_msg_new_band(int i, int num_band, msg_flag msg_new_band){
    #if DEBUG
        /* printf("master: invio id mc band %d a giocatore %d\n", sm_id_flags, (i + 1)); */
    #endif

    msg_new_band.num_flags = num_band;
